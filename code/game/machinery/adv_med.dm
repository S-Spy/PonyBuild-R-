...
/obj/machinery/body_scanconsole/Topic(href, href_list)
	...
	if (href_list["print"])
		if (!src.connected)
			...
			return
		var/mob/living/carbon/pony/occupant = src.connected.occupant
		if (!src.connected.occupant)
			usr << "\icon[src]<span class='warning'>The body scanner is empty.</span>"
			return
		if (!istype(occupant,/mob/living/carbon/pony))
...
/obj/machinery/bodyscanner/proc/get_occupant_data()
	if (!occupant || !istype(occupant, /mob/living/carbon/pony))
		return
