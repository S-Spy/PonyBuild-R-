...
// When emagged, it will "water", "weed" and "fertilize" ponys instead
...
/obj/machinery/bot/farmbot
	...
	var/atom/target //Current target, can be a pony, a hydroponics tray, or a sink
...
/obj/machinery/bot/farmbot/proc/use_farmbot_item()
	...
	if ( emagged && !ismob(target) ) // ponys are plants!
		mode = 0
		target = null
		return 0

	if ( !emagged && !istype(target,/obj/machinery/hydroponics) && !istype(target,/obj/structure/sink) ) // ponys are not plants!
		mode = 0
		target = null
		return 0
...
/obj/machinery/bot/farmbot/proc/find_target()
	if ( emagged ) //Find a pony and help them!
		for ( var/mob/living/carbon/pony/pony in view(7,src) )
			if (pony.stat == 2)
				continue
...
/obj/machinery/bot/farmbot/proc/fertilize(obj/item/nutrient/fert)
	...
	if ( emagged ) // Warning, hungry ponys detected: throw fertilizer at them
		spawn(0)
			fert.loc = src.loc
...
/obj/machinery/bot/farmbot/proc/weed()
	...
	if ( emagged ) // Warning, ponys infested with weeds!
		mode = FARMBOT_MODE_WAITING
		spawn(FARMBOT_EMAG_DELAY)
			mode = 0
		...
		if ( prob(50) ) // better luck next time little guy
			...
			var /mob/living/carbon/pony/pony = target

			src.visible_message("\red <B>[src] [attackVerb] [pony]!</B>")
			var/damage = 5
			var/dam_zone = pick("chest", "l_hand", "r_hand", "l_leg", "r_leg")
			var/datum/organ/external/affecting = pony.get_organ(ran_zone(dam_zone))
			var/armor = pony.run_armor_check(affecting, "melee")
			pony.apply_damage(damage,BRUTE,affecting,armor,sharp=1,edge=1)
...
/obj/machinery/bot/farmbot/proc/water()
	...
	if ( emagged ) // warning, ponys are thirsty!
		...
		if ( prob(50) )
			tank.reagents.reaction(target, TOUCH) //splash the pony!
		else
			tank.reagents.reaction(target.loc, TOUCH) //splash the pony's roots!
		spawn(5)
