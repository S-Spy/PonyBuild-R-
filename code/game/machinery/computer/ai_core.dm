	...
	switch(state)
		...
		if(3)
			...
			if(istype(P, /obj/item/weapon/aiModule/asimov))
				laws.add_inherent_law("You may not injure a pony being or, through inaction, allow a pony being to come to harm.")
				laws.add_inherent_law("You must obey orders given to you by pony beings, except where such orders would conflict with the First Law.")
