...
/obj/machinery/newscaster/attack_hand(mob/user as mob)            //########### THE MAIN BEEF IS HERE! And in the proc below this...############
	...
	if(istype(user, /mob/living/carbon/pony) || istype(user,/mob/living/silicon) )
		var/mob/living/pony_or_robot_user = user
		var/dat
		dat = text("<HEAD><TITLE>Newscaster</TITLE></HEAD><H3>Newscaster Unit #[src.unit_no]</H3>")

		src.scan_user(pony_or_robot_user) //Newscaster scans you
		...
		switch(screen)
			if(0)
				...
				dat+= "<BR><BR><A href='?src=\ref[pony_or_robot_user];mach_close=newscaster_main'>Exit</A>"
		...
		pony_or_robot_user << browse(dat, "window=newscaster_main;size=400x600")
		onclose(pony_or_robot_user, "newscaster_main")
...
obj/item/weapon/newspaper/attack_self(mob/user as mob)
	if(ispony(user))
		var/mob/living/carbon/pony/pony_user = user
		var/dat
		src.pages = 0
		switch(screen)
			if(0) //Cover
				...
				dat+= "<HR><DIV STYLE='float:right;'><A href='?src=\ref[src];next_page=1'>Next Page</A></DIV> <div style='float:left;'><A href='?src=\ref[pony_user];mach_close=newspaper_main'>Done reading</A></DIV>"
		...
		pony_user << browse(dat, "window=newspaper_main;size=300x400")
		onclose(pony_user, "newspaper_main")
	else
...
/obj/machinery/newscaster/proc/scan_user(mob/living/user as mob)
	if(istype(user,/mob/living/carbon/pony))                       //User is a pony
		var/mob/living/carbon/pony/pony_user = user
		if(pony_user.wear_id)                                      //Newscaster scans you
			if(istype(pony_user.wear_id, /obj/item/device/pda) )	//autorecognition, woo!
				var/obj/item/device/pda/P = pony_user.wear_id
				if(P.id)
					src.scanned_user = GetNameAndAssignmentFromId(P.id)
				else
					src.scanned_user = "Unknown"
			else if(istype(pony_user.wear_id, /obj/item/weapon/card/id) )
				var/obj/item/weapon/card/id/ID = pony_user.wear_id
