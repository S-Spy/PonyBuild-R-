datum/directive/tau_ceti_needs_women
	...
	proc/get_target_aliens()
		...
		for(var/mob/M in get_crew_of_target_gender())
			var/species = M.get_species()
			if(species == "Pegasus" || species == "Unicorn" || species == "Alicorn")
				aliens.Add(M)
		return aliens
...
datum/directive/tau_ceti_needs_women/get_description()
	return {"
		<p>
			...
			Effects were measured under laboratory and station conditions. ponys remain more trusted than Xeno. Further information is classified.
		</p>
	"}

datum/directive/tau_ceti_needs_women/initialize()
	for(var/mob/living/carbon/pony/H in get_target_heads())
		command_targets[H.wear_id] = 0

	for(var/mob/living/carbon/pony/H in get_target_aliens())
		alien_targets.Add(H.wear_id)

	special_orders = list(
		"Remove [get_target_gender()] personnel from Command positions.",
		"Terminate employment of all [get_target_gender()] Alicorn, pegasus, and Unicorn.")
