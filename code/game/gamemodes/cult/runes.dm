...
/////////////////////////////////////////EIGHTH RUNE
		...
		raise()
			var/mob/living/carbon/pony/corpse_to_raise
			var/mob/living/carbon/pony/body_to_sacrifice

			var/is_sacrifice_target = 0
			for(var/mob/living/carbon/pony/M in src.loc)
				if(M.stat == DEAD)
			...
			find_sacrifice:
				for(var/obj/effect/rune/R in world)
					if(R.word1==cultwords["blood"] && R.word2==cultwords["join"] && R.word3==cultwords["hell"])
						for(var/mob/living/carbon/pony/N in R.loc)
...
/////////////////////////////////////////TENTH RUNE
		...
		ajourney() //some bits copypastaed from admin tools - Urist
			if(usr.loc==src.loc)
				var/mob/living/carbon/pony/L = usr
...
/////////////////////////////////////////ELEVENTH RUNE
		...
		manifest()
			...
			var/mob/living/carbon/pony/dummy/D = new(this_rune.loc)
...
/////////////////////////////////////////FIFTEENTH RUNE
		...
		sacrifice()
			var/list/mob/living/carbon/pony/cultsinrange = list()
			var/list/mob/living/carbon/pony/victims = list()
			for(var/mob/living/carbon/pony/V in src.loc)//Checks for non-cultist ponys to sacrifice
				if(ispony(V))
					if(!(iscultist(V)))
			...
			for(var/mob/H in victims)
				...
				if(istype(H,/mob/living/carbon/pony))
					var/mob/living/carbon/pony/lamb = H
...
/////////////////////////////////////////NINETEENTH RUNE
		...
		cultsummon()
			...
			if(users.len>=3)
				...
				for(var/mob/living/carbon/pony/C in users)
					if(iscultist(C) && !C.stat)
...
//			var/list/cultboil = list(cultists-usr) //and for this words are destroy-see-blood.
			...
			if(cultists.len>=3)
				...
				for(var/mob/living/carbon/pony/C in orange(1,src))
					if(iscultist(C) && !C.stat)
