...
/datum/game_mode/epidemic/post_setup()
	...
	var/list/crew = list()
	for(var/mob/living/carbon/pony/H in world) if(H.client)
	...
	for(var/i = 0, i < lethal_amount, i++)
		var/mob/living/carbon/pony/H = pick(crew)
	...
	var/mob/living/carbon/pony/patient_zero = pick(infectees)
...
/datum/game_mode/epidemic/check_win()
	var/alive = 0
	var/sick = 0
	for(var/mob/living/carbon/pony/H in world)
