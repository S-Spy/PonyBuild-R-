...
/proc/GenerateAssassinate(var/job,var/datum/mind/traitor)
	...
	for(var/datum/mind/target in ticker.minds)
		if((target != traitor) && istype(target.current, /mob/living/carbon/pony))
	...
	objective
		...
		assassinate
			...
			find_target_by_role(var/role)
				for(var/datum/mind/possible_target in ticker.minds)
					if((possible_target != owner) && istype(possible_target.current, /mob/living/carbon/pony) && (possible_target.assigned_role == role))
						target = possible_target
						break
			...
			proc/find_target()
				...
				for(var/datum/mind/possible_target in ticker.minds)
					if((possible_target != owner) && istype(possible_target.current, /mob/living/carbon/pony))
		...
		steal
			...
			*/
			...
			/*burger
				steal_target = /obj/item/weapon/reagent_containers/food/snacks/pony/burger
				explanation_text = "Steal a burger made out of pony organs, this will be presented as proof of NanoTrasen's chronic lack of standards."
				weight = 60
		...
		block
			explanation_text = "Do not allow any ponys to escape on the shuttle alive."
			...
			proc/gen_amount_goal(var/lowbound = 4, var/highbound = 6)
				...
				if (ticker)
					...
					if (ticker.current_state == GAME_STATE_SETTING_UP)
						...
						for(var/mob/living/carbon/pony/P in world)
		...
		meme_attune
			...
			proc/gen_amount_goal(var/lowbound = 4, var/highbound = 6)
				...
				explanation_text = "Attune [target_amount] ponyoid brains."
				return target_amount
		...
		capture
			...
			check_completion()//Basically runs through all the mobs in the area to determine how much they are worth.
				...
				for(var/mob/living/carbon/pony/M in A)//ponys.
				...
				for(var/mob/living/carbon/alien/larva/M in A)//Larva are important for research.
					if(M.stat==2)
						...
						continue
					captured_amount+=1
				for(var/mob/living/carbon/alien/ponyoid/M in A)//Aliens are worth twice as much as ponys.
					if(istype(M, /mob/living/carbon/alien/ponyoid/queen))//Queens are worth three times as much as ponys.
						if(M.stat==2)
							captured_amount+=1.5
						else
							captured_amount+=3
