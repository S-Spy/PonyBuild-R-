	...
	if(useMS)
		if(prob(5))
			...
			switch(pick(1,2,3,4,5,6,7))
				...
				if(2)
					sender = pick(300;"QuickDatingSystem",200;"Find your russian bride",50;"Pegasus beauties are waiting",50;"Find your secret alicorn crush",50;"Beautiful unicorn brides")
				...
				if(6)
					sender = pick("NanoTrasen Morale Divison","Feeling Lonely?","Bored?","www.wetalicorn.nt")
					message = pick("The NanoTrasen Morale Division wishes to provide you with quality entertainment sites.",\
					"WetAlicorn.nt is a xenophillic website endorsed by NT for the use of male crewmembers among it's many stations and outposts.",\
					"Wetalicorn.nt only provides the higest quality of male entertaiment to NanoTrasen Employees.",\
