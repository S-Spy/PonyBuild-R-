...
/obj/machinery/atm/proc/scan_user(mob/living/carbon/pony/pony_user as mob)
	if(!authenticated_account)
		if(pony_user.wear_id)
			var/obj/item/weapon/card/id/I
			if(istype(pony_user.wear_id, /obj/item/weapon/card/id) )
				I = pony_user.wear_id
			else if(istype(pony_user.wear_id, /obj/item/device/pda) )
				var/obj/item/device/pda/P = pony_user.wear_id
				I = P.id
			if(I)
				authenticated_account = attempt_account_access(I.associated_account_number)
				if(authenticated_account)
					pony_user << "\blue \icon[src] Access granted. Welcome user '[authenticated_account.owner_name].'"
...
/obj/machinery/atm/proc/release_held_id(mob/living/carbon/pony/pony_user as mob)
	if(!held_card)
		return
	...
	if(ispony(pony_user) && !pony_user.get_active_hand())
		pony_user.put_in_hands(held_card)
	held_card = null


/obj/machinery/atm/proc/spawn_ewallet(var/sum, loc, mob/living/carbon/pony/pony_user as mob)
	var/obj/item/weapon/spacecash/ewallet/E = new /obj/item/weapon/spacecash/ewallet(loc)
	if(ispony(pony_user) && !pony_user.get_active_hand())
		pony_user.put_in_hands(E)
	E.worth = sum
