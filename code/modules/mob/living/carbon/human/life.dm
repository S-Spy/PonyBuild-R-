...
#define pony_MAX_OXYLOSS 1 //Defines how much oxyloss ponys can get per tick. A tile with no air at all (such as space) applies this value, otherwise it's a percentage of it.
#define pony_CRIT_MAX_OXYLOSS ( (last_tick_duration) /6) //The amount of damage you'll get when in critical condition. We want this to be a 5 minute deal = 300s. There are 50HP to get through, so (1/6)*last_tick_duration per second. Breaths however only happen every 4 ticks.
...
// Calculate how vulnerable the pony is to under- and overpressure.
// Returns 0 (equals 0 %) if sealed in an undamaged suit, 1 if unprotected (equals 100%).
// Suitdamage can modifiy this in 10% steps.
/mob/living/carbon/pony/proc/get_pressure_weakness()
...
// Calculate how much of the enviroment pressure-difference affects the pony.
/mob/living/carbon/pony/calculate_affecting_pressure(var/pressure)
	...
	// The return value will be the adjusted_pressure of the pony that is the basis of pressure warnings and damage.
		...
		if (radiation)
			...
			if (radiation > 50)
				...
				if(prob(5) && prob(100 * RADIATION_SPEED_COEFFICIENT))
					...
					if(!lying)
						emote("collapse")
				if(prob(5) && prob(100 * RADIATION_SPEED_COEFFICIENT) && species.name == "Earthpony") //apes go bald
	...
	proc/handle_breath(datum/gas_mixture/breath)
		...
		if(!breath || (breath.total_moles == 0) || suiciding)
			...
			if(health > config.health_threshold_crit)
				adjustOxyLoss(pony_MAX_OXYLOSS)
			else
				adjustOxyLoss(pony_CRIT_MAX_OXYLOSS)
		...
		if(inhale_pp < safe_pressure_min)
			...
			// Don't fuck them up too fast (space only does pony_MAX_OXYLOSS after all!)
			adjustOxyLoss(max(pony_MAX_OXYLOSS*(1-ratio), 0))
			failed_inhale = 1
...
/mob/living/carbon/pony/proc/handle_hud_list()
...
/mob/living/carbon/pony/handle_silent()
	if(..())
		speech_problem_flag = 1
	return silent

/mob/living/carbon/pony/handle_slurring()
	if(..())
		speech_problem_flag = 1
	return slurring

/mob/living/carbon/pony/handle_stunned()
...
/mob/living/carbon/pony/handle_stuttering()
	if(..())
		speech_problem_flag = 1
	return stuttering

/mob/living/carbon/pony/handle_fire()
	if(..())
		return
...
#undef pony_MAX_OXYLOSS
#undef pony_CRIT_MAX_OXYLOSS
