...
/datum/surgery_step
	...
	proc/is_valid_target(mob/living/carbon/pony/target)
		if(!hasorgans(target))
			return 0
	...
	proc/can_use(mob/living/user, mob/living/carbon/pony/target, target_zone, obj/item/tool)
		return 0

	// does stuff to begin the step, usually just printing messages. Moved germs transfering and bloodying here too
	proc/begin_step(mob/living/user, mob/living/carbon/pony/target, target_zone, obj/item/tool)
		var/datum/organ/external/affected = target.get_organ(target_zone)
		if (can_infect && affected)
			spread_germs_to_organ(affected, user)
		if (ispony(user) && prob(60))
	...
	proc/end_step(mob/living/user, mob/living/carbon/pony/target, target_zone, obj/item/tool)
		return

	// stuff that happens when the step fails
	proc/fail_step(mob/living/user, mob/living/carbon/pony/target, target_zone, obj/item/tool)
		return null

proc/spread_germs_to_organ(datum/organ/external/E, mob/living/carbon/pony/user)
