...
/obj/item/weapon/archaeological_find/New(loc, var/new_item_type)
	...
	switch(find_type)
		...
		if(27)
			...
			//33% chance to be able to reload the gun with pony ammunition
			if(prob(66))
				new_gun.caliber = "999"
		...
		if(32)
			//ponyoid remains
			apply_prefix = 0
			item_type = "ponyoid [pick("remains","skeleton")]"
			icon = 'icons/effects/blood.dmi'
			icon_state = "remains"
			additional_desc = pick("They appear almost pony.",\
	...
	if(apply_image_decorations)
		...
		[pick("an alien ponyoid","an amorphic blob","a short, hairy being","a rodent-like creature","a robot","a primate","a reptilian alien","an unidentifiable object","a statue","a starship","unusual devices","a structure")] \
		[pick("surrounded by","being held aloft by","being struck by","being examined by","communicating with")] \
		[pick("alien ponyoids","amorphic blobs","short, hairy beings","rodent-like creatures","robots","primates","reptilian aliens")]"
		if(prob(50))
