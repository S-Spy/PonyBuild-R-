...
mob/living/carbon/proc/pain(var/partname, var/amount, var/force, var/burning = 0)
	...
	if(world.time < next_pain_time && !force)
		return
	if(amount > 10 && istype(src,/mob/living/carbon/pony))
		if(src:paralysis)
			src:paralysis = max(0, src:paralysis-round(amount/10))
	if(amount > 50 && prob(amount / 5))
		src:drop_active_hand()
	var/msg
	if(burning)
		switch(amount)
			if(1 to 10)
...
mob/living/carbon/pony/proc/custom_pain(var/message, var/flash_strength)
	if(stat >= 1) return
...
mob/living/carbon/pony/proc/handle_pain()
	// not when sleeping
