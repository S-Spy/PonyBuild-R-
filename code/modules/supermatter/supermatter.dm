...
/obj/machinery/power/supermatter/process()
	...
	for(var/mob/living/carbon/pony/l in view(src, min(7, round(sqrt(power/6))))) // If they can see it without mesons on.  Bad on them.
...
/obj/machinery/power/supermatter/proc/supermatter_pull()
	...
	for(var/atom/X in orange(pull_radius,src))
		...
		if(istype(X, /atom/movable))
			...
			if(((X) && (!istype(X,/mob/living/carbon/pony))))
				step_towards(X,src)
