		...
		if (!( usr.restrained() || (usr.lying && usr.buckled!=src) ))
			...
			if (W)
				...
				if (istype(usr, /mob/living/carbon/pony))
					...
					// ------- YOU ARE NOT pony. WHAT ARE YOU - DETERMINED HERE AND PROPER ATTACK_MOBTYPE CALLED -------
					if (istype(usr, /mob/living/carbon/monkey))
						src.attack_paw(usr, usr.hand)
					else if (istype(usr, /mob/living/carbon/alien/ponyoid))
						if(usr.m_intent == "walk" && istype(usr, /mob/living/carbon/alien/ponyoid/hunter))
							usr.m_intent = "run"
